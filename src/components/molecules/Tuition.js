...
async function getApi(){
    try {
        const response = await axios.get("https://pluralcode.academy/pluralcode_apis/api/list_courses_details");
  ...
  return (
    ...
    <div className="pad">
      ...
      <p className="navtextcolor tuitp">Pluralcode often has discount scholarship programs running from time to time, if you hurry, you may qualify for one.</p>
      <div className="text-center py-5">

      <Link to="beginen" smooth={true}><Button className="btn button px-5 pbcolor" children="Begin Enrollment"/>
      </Link>
      </div>
    </div>
      </Element>
